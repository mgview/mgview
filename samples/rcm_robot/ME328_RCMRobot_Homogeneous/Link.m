...
circleDepth                     =  10;                     % cm                  Constant
circleRadius                    =  20;                     % cm                  Constant
density                         =  2.7;                    % g/(cm^3)            Constant
kdx                             =  0;                      % N*m*s               Constant
kdy                             =  0;                      % N*m*s               Constant
kdz                             =  0;                      % N*s/m               Constant
kpx                             =  0;                      % N*m                 Constant
kpy                             =  0;                      % N*m                 Constant
kpz                             =  0;                      % N/m                 Constant
...
rLink                           =  3;                      % cm                  Constant
thetaStart                      =  45;                     % deg                 Constant
traceFrequency                  =  1;                      % rev/s               Constant

dz                              =  0.25;                   % m                   Initial Value
qx                              =  30;                     % deg                 Initial Value
qy                              =  30;                     % deg                 Initial Value
...
integStp                        =  0.05;                   % s                   Integration Step
...
DEGtoRAD = pi / 180.0;
RADtoDEG = 180.0 / pi;
circleDepth = circleDepth * 0.01;
circleRadius = circleRadius * 0.01;
density = density * 999.9999999999999;
lOffset = lOffset * 0.01;
rLink = rLink * 0.01;
thetaStart = thetaStart * DEGtoRAD;
traceFrequency = traceFrequency * 6.283185307179586;
qx = qx * DEGtoRAD;
qy = qy * DEGtoRAD;
...
y = circleRadius*sin(thetaStart+traceFrequency*t);
z = -circleDepth;
qxDesired = asin(y/sqrt(y^2+z^2));
zp = 0;
yp = circleRadius*traceFrequency*cos(thetaStart+traceFrequency*t);
qxDesiredp = -z*(y*zp-z*yp)/(cos(qxDesired)*(y^2+z^2)^1.5);
Tx = kpx*(qxDesired-qx) + kdx*(qxDesiredp-qxp);
x = circleRadius*cos(thetaStart+traceFrequency*t);
qyDesired = asin(x/sqrt(x^2+y^2+z^2));
xp = -circleRadius*traceFrequency*sin(thetaStart+traceFrequency*t);
...
if( isempty(Output) ),
   if( ~isempty(FileIdentifier) ),
      ...
      fprintf( 1, '\n Output is in the files Link.i  (i=1, ..., 7)\n\n' );
   end
...
if( isempty(hasHeaderInformationBeenWritten) ),
   ...
   if( shouldPrintToFile && isempty(FileIdentifier) ),
      ...
      FileIdentifier(7) = fopen('Link.7', 'wt');   if( FileIdentifier(7) == -1 ), error('Error: unable to open file Link.7'); end
      fprintf(FileIdentifier(7), '%% FILE: Link.7\n%%\n' );
      fprintf(FileIdentifier(7), '%%       t          dzDesired         dz\n' );
      fprintf(FileIdentifier(7), '%%      (s)            (m)            (m)\n\n' );
   end
...
if( shouldPrintToFile ),   WriteNumericalData( FileIdentifier(7), Output(79:81) );  end
end
...
legend( 'dzDesired (m)', 'dz (m)' );
xlabel('t s');   % ylabel('Enter y-axis label');   title('Enter plot title');
clear data;
end
